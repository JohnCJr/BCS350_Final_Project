...
if (isset($_POST['assignmentid'])) $assignid = sanitize($mysqli, $_POST['assignmentid']);
else $assignid = NULL;
if (isset($_POST['pathname'])) $pathname = sanitize($mysqli, $_POST['pathname']);
else $pathname = NULL;
if (isset($_POST['task'])) $task = sanitize($mysqli, $_POST['task']);
else $task = NULL;
if (isset($_POST['classname'])) $class = sanitize($mysqli, $_POST['classname']);
else $class = NULL;
if ($class == 'All') $class = NULL;
if (isset($_POST['sort'])) $sort = $_POST['sort'];
else $sort = 'Select';
if ($sort == 'Select') $sort = NULL;
if (isset($_POST['sortdirection'])) $sortdirection = $_POST['sortdirection'];
else $sortdirection = 'Select';
if ($sortdirection == 'Select') $sortdirection = NULL;

// Get photo from student table for student currently logged in and sets the $profilepic and $contact variable
...
// Process class input
if ($class != NULL) $where = " WHERE class.classname = '$class' AND $table.studentid = $currentid";
else $where = " WHERE $table.studentid = $currentid";
...
if ($task == 'Upload') {
    if (isset($_FILES['document']['tmp_name']) && ($assignid != NULL)) {
        list($rc, $filename) = upload('document', $dir, "$currentid-$assignid", $extns, $maxsize);
        if ($rc == 0) {
            $query = "UPDATE studentassignment SET submission = '$filename' 
                      WHERE studentid = '$currentid' AND assignmentid = '$assignid'";
            $result = mysqli_query($mysqli, $query);
            if (!$result) {
                $msg = "File Uploaded Successfully.  RC: $rc currentid: $currentid";
            }
        } else {
    ...
    if (!isset($_FILES['document']['tmp_name'])) {
        $msg .= 'Please upload a document if you wish to submit an assignment<br>';
    }
}
...
while (list($classname, $assignmentname, $tfirstname, $tlastname, $email, $submitted, $grade, $assignmentid, $submission) = mysqli_fetch_row($result)) {
    ...
    if ($submission == NULL) {
        ...
        echo "<input type='hidden' name='pathname' value='$submission'>
              <td style='background-color: lightgreen;'><input type='submit' name='task' value='View submission'></td>";
    }
